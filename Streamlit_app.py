...
def main():
    ...
    if st.session_state.password_flag:
        ...
        selected_time = st.selectbox('How much time do you have?', ['', '30 min', '1 hr', '>1 hr', 'unlimited'], index=0)
        weather = st.selectbox('What is the weather like?', ['', 'Normal', 'Hot', 'Raining', 'About to rain'], index=0)
        mood = st.selectbox('How are you feeling?', ['', 'adventurous', 'stressed', 'at peace', 'tired', 'hopeful'], index=0)
        additional_requirements = st.text_area('Anything else I should know? Special needs / wishes / cravings?')

        if st.button("Get Makan Mama's expert advice"):
            if not selected_time or not weather:
                st.warning('Please answer all questions.')
            
            else:
                user_context = f"Time available: {selected_time}. Weather: {weather}. Additional requirements: {additional_requirements}. Mood: {mood}."
